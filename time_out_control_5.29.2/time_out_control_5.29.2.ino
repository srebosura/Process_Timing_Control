    ...
    double SensInTime = 0;
    double SensInLastCheck = millis();
    double SensOutTime = 0;
    double SensOutLastCheck = millis();
    double Sens1Time = 0;
    ...
    void setup()  {
       ...
       if (digitalRead(start_buttonPin)==0)  {
        ...
        int sens_default_time = 10; //default frame edge detect time-out
       
        EEPROM.put(EE_addr1, default_time);
        EEPROM.put(EE_addr2, sens_default_time);
        }
      //read stored settings
      EEPROM.get(EE_addr1,EEPROM_read_time);
      EEPROM.get(EE_addr2,EEPROM_read_sens_time);
      //EEPROM_read_time = 30; //for simulation
      //EEPROM_read_sens_time = 5; // for simulation
      time_var = EEPROM_read_time; 
      sens_time_var = EEPROM_read_sens_time;     
      
       scroll("Starting up ....",10,1);
       lcd.begin(16, 2);
       lcd.print("TimerSetting=");
       lcd.print(time_var);
       lcd.setCursor(0,1);
       lcd.print("FrameTimeOut=");
       lcd.print(sens_time_var);
       delay(1000);     
       }
...
//main menu actions
 ...
 if(start_buttonState==LOW && menu_display==0){
  ...
  tone( piezoPin, 2100, 250);
  lastButtonPushed=4; 
 }else if(start_buttonState==LOW && menu_display==4){
  tone( piezoPin, 2100, 250);
  lastButtonPushed=5;
  ...
  tone( piezoPin, 2100, 250);
  lastButtonPushed=6;
 }else if(select_buttonState==LOW && menu_display==6){
  tone( piezoPin, 2100, 250);
  lastButtonPushed=0; 
 }else if(start_buttonState==LOW && (lastButtonPushed==3 || lastButtonPushed==7 )){
  tone( piezoPin, 2100, 250);
  lastButtonPushed=2;
 }else if(start_buttonState==LOW && (lastButtonPushed==5 || lastButtonPushed==7 )){
  tone( piezoPin, 2100, 250);
  lastButtonPushed=4;          
 }
//Sensor actions
    ...
    lcd.setCursor(0, 0);
    lcd.print("F DetectIN      ");
    tone( piezoPin, 1750, 200);
    if (SensIn1Flag == HIGH && SensIn2Flag == HIGH &&  SensIn3Flag == HIGH && SensIn4Flag == HIGH) {
      return;
    }
    SensInFlag = SensInFlag + 1;
    SensInTime = 0;
    SensInLastCheck = millis();
    ...
    lcd.setCursor(0, 0);
    lcd.print("FDetectOUT      ");
    tone( piezoPin, 1750, 200);
    if (SensOut1Flag == HIGH && SensOut2Flag == HIGH &&  SensOut3Flag == HIGH && SensOut4Flag == HIGH) {
      return;
    }
    SensOutFlag = SensOutFlag + 1;
    SensOutTime = 0;
    SensOutLastCheck = millis();
...
void set_sens_time() {
   ...
   EEPROM.put(EE_addr2, sens_time_var);
   lcd.clear();
   EEPROM.get(EE_addr2,EEPROM_read_sens_time);
   sens_time_var = EEPROM_read_sens_time; 
   lcd_display(0,0,"Settings Saved!"," ");
 ...
 void read_user_set() {
  ...
  void set_menu_time() {
      ...
      delay(2000);
      while (lastButtonPushed==3 || lastButtonPushed==7) {
        read_inputs();
  ...
  void set_menu_sens_time() {
      run_status = 3;
      lcd.setCursor(0, 0);
      lcd.print("HIT  ENT to SAVE");
      lcd.setCursor(0, 1);
      lcd.print("FrameTimeOutS=");
      sens_time_var = EEPROM_read_sens_time ;
      lcd.print(sens_time_var);
      delay(2000);
      while (lastButtonPushed==5 || lastButtonPushed==7) {
        read_inputs();
        if (digitalRead(select_buttonPin)== 0) {
          delay(250);
          sens_time_var = sens_time_var+1;
        }
        if (digitalRead(stop_buttonPin)== 0) {
          delay(250);
          sens_time_var = sens_time_var-1;
 ...
 for (int rep = 0; rep < repeat; rep++)  { 
  ...
  void main_menu() {
   ...
   case 4:
      ...
      lcd.setCursor(0, 1);
      lcd.print("FrameTimeOutS=");
      lcd.print(EEPROM_read_sens_time,1);
      menu_display = 4;
      last_menu = menu_display;
      delay(150);
      break;
  ...
  void set_sens3() {
    ...
    SensOut3Flag = 0;
    lcd.setCursor(0, 0);
    lcd.print("FRAME3 SET");
    tone( piezoPin, 1950, 150);         
  }
  void set_sens4() {
    Serial.println("SensIn4 Flag = ");
    Serial.println(SensIn4Flag);
    Serial.println("F4 is set");
    Sens4Time = time_var*1000;
    Sens4LastCheck = millis();
    frame4 = HIGH;
    SensOut4Flag = 0;
    lcd.setCursor(0, 0);
    lcd.print("FRAME4 SET");
    tone( piezoPin, 1950, 150);         
  }
  void reset_sens1() {
    SensIn1Flag = 0;
  ...
  void reset_sens3() {
    ...
    frame3 = LOW;
    lcd.setCursor(0, 0);
    lcd.print("FRAME3 RST"); 
    tone( piezoPin, 2400, 150);
    if (SensIn1Flag == 0 && SensIn2Flag == 0 && SensIn3Flag == 0) {
      SensOutFlag = 0;
      SensInFlag = 0;
    }
    frame_counter = frame_counter+1;            
  ...
  void reset_sens4() {
    SensIn4Flag = 0;
    Serial.println("F4 reset.....");
    frame4 = LOW;
    lcd.setCursor(0, 0);
    lcd.print("FRAME4 RST"); 
    tone( piezoPin, 2400, 150);
    frame_counter = frame_counter+1;            
  }
  void update_sens1() {
  ...
  void update_sens4() {
    Sens4Time -= ((millis() - Sens4LastCheck));
    if (floor (Sens4Time/1000) == 99) {
      lcd.setCursor(11, 0);
    lcd.print("4=   ");
      }
    if (floor(Sens4Time/1000) == 9) {
      lcd.setCursor(11, 0);
    lcd.print("4=   ");
      }  
    lcd.setCursor(11, 0);
    lcd.print("4=");
    lcd.print(Sens4Time/1000,0);
    Sens4LastCheck = millis();
    if (Sens4Time <= 0) {
      error_msg = "  Error Frame 4 ";
      error_alarm();
      return;  
    }
    Serial.println("F4 updated");
    Serial.println(SensIn4Flag);
  }
  void check_sensor1() {
    //Serial.println("SensIn1Flag = ");
  ...
  void check_sensor4() {
    ...
    if (SensIn4Flag == 1 && frame4 == LOW) {
      set_sens4();  
    }
    if (SensOut4Flag == 1 && frame4 == HIGH) {
      reset_sens4();
    }
    if (frame4 == HIGH) {
     update_sens4();   
  ...
  void sensor_manager() {
    if (run_status == 1) {
      ...
      if (SensOutFlag == 1 || SensOutFlag == 3 || SensOutFlag == 5 || SensOutFlag == 7)  {
      SensOutTime = ((millis() - SensOutLastCheck));
        if (SensOutTime >= sens_time_var*1000) {
          error_msg = "Bad Frame OUT ER";
          run_status = 0;
          error_alarm();
        } 
       }
     if (SensInFlag == 1 || SensInFlag == 3 || SensInFlag == 5 || SensInFlag == 7)  {
      SensInTime = ((millis() - SensInLastCheck));
        if (SensInTime  >= sens_time_var*1000) {
         error_msg = "Bad Frame IN ERR";
         run_status = 0;
         error_alarm();  
       }
    }   
